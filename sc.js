...
(async () => {
  ...
  const scrollUntilAllElementsLoaded = async () => {
    let previousCount = 0;
    while (true) {
      // Скролимо вниз
      await page.evaluate(() => {
      ...
      // Якщо кількість елементів не змінилася, завершуємо цикл
  ...
  const getData = async () => {
    await scrollUntilAllElementsLoaded();
  ...
  if (
    ...
    // Перетворюємо результати в формат таблиці
