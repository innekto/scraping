...
(async () => {
  ...
  await page.goto(
    ...
    {
      waitUntil: 'networkidle2', // Чекаємо, поки сторінка повністю завантажиться
    },
  );

  // Функція для підрахунку кількості елементів
  ...
  // Функція для скролу та перевірки нових елементів
  const scrollUntilAllLoaded = async () => {
    let previousCount = await getItemCount();
    ...
    while (true) {
      ...
      await new Promise((resolve) => setTimeout(resolve, 120000)); // Збільшено затримку

      // Отримуємо нову кількість елементів
      currentCount = await getItemCount();

      // Перевіряємо, чи кількість нових елементів збільшилася
      if (currentCount === previousCount) {
        break; // Немає нових елементів, закінчуємо
      }
  ...
  // Перевіряємо довжину масиву результатів
